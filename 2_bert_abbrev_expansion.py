...
from bert_utils import get_torch_device
import logging, sys, json

import pandas as pd
from abbr_utils import ExpansionInstance, BestFitPolicy, choose_best_fit, get_lang_model_predictions
...
def unify_predictions(predictions: List[ExpansionInstance]) -> List[Dict]:
    ...
    for pred in predictions:
        doc_id, parag_id, s_id, sent_mask_part = pred.sent_id.split('.') # sbl00756.p1.s147.0
...
def evaluate_expansion_predictions(predictions: List[ExpansionInstance]):
    ...
    for pred in predictions:
        ...
        # Soft analysis, we take it as a match if the candidate is IN The Prediciton List (we can figure out later some heuristics to try to get it form the list)
        # YET WE COULD GO SOFTER! Levenshtein ditance to the abbr candidate? partial character match? Just to pick something...
    ...
    pd.DataFrame(error_data).to_csv('abbreviations/data/ERRORS.csv', columns=['gold', 'prediction', 'query'])
    pd.DataFrame(soft_error_list).to_csv('abbreviations/data/ERRORS_NO-TOPN.csv')
...
if __name__ == '__main__':
    ...
    file_hdlr = logging.FileHandler(filename=f"{MODEL_OUT_DIR}/bert_abbrev_expansion.{DATA_PARTITION}.log")
    logging.basicConfig(level=logging.INFO, handlers=[console_hdlr, file_hdlr])

    # Load Dataset
    test_data = load_abbreviations_dataset(f"data/sbl-51abbr.masked.upperbound.preexp.{DATA_PARTITION}.json")


    #### Fill-in Masked abbreviation with BERT predictions ...
    ## [logging.info(x['Sentence']) for x in dataset]
    predictions, pred_dict = get_lang_model_predictions(test_data, BERT_MODEL_NAME, BERT_TOKENIZER_NAME, TOP_K, GPU_IX)
    pd.DataFrame([p._asdict() for p in predictions]).to_json(f'{MODEL_OUT_DIR}/model_raw_predictions.{DATA_PARTITION}.jsonl', orient='records')
    # Save Predictions
    unified_predictions = unify_predictions(predictions)
    pd.DataFrame(unified_predictions).to_json(f'{MODEL_OUT_DIR}/model_unified_predictions.{DATA_PARTITION}.jsonl')

    # Save Accumulated Per-Candidate Predictions 
    unique_vals_preds = {k: Counter(v).most_common(10) for k,v in pred_dict.items()}
    json.dump(unique_vals_preds, open(f"{MODEL_OUT_DIR}/bert_prediction_mapping.{DATA_PARTITION}.json", "w"), indent=2)
